...
- GitHub session reports, including zero matches and partial/failed sessions. Zero-job recovery checks update the existing window's report without another issue/comment notification. An initial local seven-day report can be acknowledged through additive receipts without resetting hosted discovery history. There is no rolling queue. [Daily workflow](DAILY_WORKFLOW.md).
...
For daily use, open [session report issues](https://github.com/prudhviy99/h1b-job-monitor/issues?q=is%3Aissue+label%3Ah1b-session-report). Each morning/evening window gets a report, even with zero new matches. Recovery checks with no new jobs are appended to that window's existing issue without notification comments; recovery checks that find jobs create a new issue so those jobs are not missed. Issues contain the session time in Pacific time, source health, results and a link to downloadable HTML/CSV/JSON reports. Previous reports remain snapshots; their contents do not disappear when a job is absent from a later scan. Check the official page before applying because a historical report is not a claim that a role remains open.
...
This repository is intentionally public. Its workflow definition, run logs, job summaries, report/failure issues, repository owner, and legacy local launcher/package identifiers are therefore public. The checked-in matching profile contains only generalized experience and skill evidence; the resume file, contact details, and local SQLite database are excluded. Each actual crawl retains its report artifact for 30 days and publishes to an owner-assigned public session-report issue. The first check in each window creates an issue even with zero P0/P1/P2 matches; zero-job recovery checks update it. GitHub notification delivery follows the owner's account settings. Scheduler wake-ups that do not crawl do not create report issues. A retry of the same GitHub workflow replaces only its own report section rather than creating another issue or erasing earlier jobs.
...
Issues labeled **h1b-session-report** preserve every actual crawl, including zero-match and failed sessions; reports containing jobs also receive **h1b-monitor-match**. Neither is automatically closed. A usable partial crawl is clearly marked **partial** in its report and **needs attention** in the status issue, but is not also marked as a crashed GitHub workflow or duplicated in a separate failure issue. This prevents redundant Actions-failure emails for individual source problems. Separate **h1b-monitor-failure** alerts remain for crashed/wholly failed crawls and infrastructure problems; repeat updates edit the body without adding notification comments. Recovery closes those alerts, not historical reports. State/report upload failures still fail the workflow. Closure does not delete or retract jobs. All new issue dates and visible crawl timestamps use Pacific time. Expedia and other sites can intermittently deny hosted requests; the crawler stops that source's detail requests on 401/403, preserves its cursor, and reports the failure without bypassing access controls. Malformed sitemap XML gets one rate-limited refresh; persistent parse errors remain incomplete. The former queue issue is retired; its history is preserved.
